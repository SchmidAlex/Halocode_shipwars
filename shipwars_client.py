...
@event.start
def on_start():
    halo.mesh.join_group('alex')

def cursorHandling():
    global background, xPos, yPos, cursorActive, isCursorLightOn, adminReady
    while cursorActive and adminReady:
...
def placeShips():
    global background, xPos, yPos, isCursorLightOn
...
def refreshBackground():
    global background, xPos, yPos, cursorActive, playerShootX, playerShootY, gameStarted
    if not cursorActive:
...
def on_button_pressed():
    global xPos, yPos, background, myShips, gameStarted, cursorActive, isCursorLightOn
    if gameStarted and cursorActive:
        ...
        if (len(myShips) == 10):
            ...
            halo.mesh.broadcast('game_start')
            gameStarted = True
            return

@event.mesh_message('ready')
def on_mesh_message():
    global adminReady, cursorActive
    adminReady = True
    cursorActive = True
    placeShips()
